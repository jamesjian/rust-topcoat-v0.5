...
# Topcoat v0.5.0 — Simple Web Application with MySQL/MariaDB


A minimal web application built with **Topcoat v0.5.0** that connects to a MySQL/MariaDB database and lists records from a table.
...
**Default URL:** [http://127.0.0.1:3000/products](http://127.0.0.1:3000/products)
...
```
main.rs
  → loads DATABASE_URL from .env via the dotenvy crate
    ↓
app.rs
  → builds app_context and passes the database connection through the router
    ↓
handlers/frontend/home.rs → products()
  → acts as the controller, handles the incoming request
    ↓
templates/frontend/t_home.rs → ProductList()
  → uses the sqlx crate to query the database and render the page
```
